...
- [ ] T20 — GATE: `flutter test` — all 18 new SCENARIOs pass; total suite green (no regressions)
- [ ] T21 — VERIFY: no hex literals in any new file; no `PhosphorIcons.X` direct usage; all spacing values from scale (8/12/14/18/20); all colors via `AppPalette.of(context)` or `AppPalette.of(context).X`; all icons via `TreinoIcon.X`; `FriendRequestsInboxScreen` adds no `Scaffold`/`AppBackground`/`SafeArea`
