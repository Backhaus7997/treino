...
> ## 🚨 Los pasos `[MANUAL]` de este archivo escriben en PRODUCCIÓN
>
> `treino-dev` es el **único** proyecto Firebase de TREINO — no existe
> `treino-prod`, no hay entorno de nube descartable. El nombre dice "dev" por
> historia (el project ID no se puede cambiar) y adentro viven los usuarios
> reales. **Que el comando diga `--project treino-dev` no es un aviso: es
> exactamente el nombre que engaña.**
>
> Este change está **abierto** — no tiene `archive-report.md`. Los `[x]` de acá
> son intención registrada, no evidencia de ejecución: `apply-progress.md`
> dice, para PR2, *"ningún script se corrió contra datos reales todavía, ni
> siquiera en `--dry-run`"*, y para PR3a que los pasos 3a.19b–3a.22 *"siguen
> pendientes"*. **Leelo antes de dar por corrido cualquier ítem de abajo.**
>
> Los pasos marcados `🚨 PROD` abajo son escrituras: `firebase deploy` (rules,
> índices, functions), `--apply` de los scripts de `functions/scripts/`
> (Admin SDK → **saltea las rules**, incluido el freeze de M-03b) y un
> `functions:delete`. Los `--dry-run`, el `.count()` y `verify --cutover` son
> de lectura y no llevan marca. **Ninguno lo corre un agente**: van a mano, con
> OK explícito de un humano.
>
> Ver [openspec/AGENTS.md](../../AGENTS.md) ·
> [AGENTS.md § Entornos](../../../AGENTS.md#-entornos--leer-antes-de-correr-cualquier-comando) ·
...
- [x] **1.32 [MANUAL]** 🚨 PROD (ver banner arriba) — `firebase deploy --only firestore:rules,firestore:indexes --project treino-dev` (M-03). Deploy seguro por construcción: aditivo, nada lo lee todavía (LD-10).
...
- [x] **3a.20 [MANUAL]** 🚨 PROD (ver banner arriba) — `firebase deploy --only firestore:rules --project treino-dev` (M-07, **sólo el flip direccional del gate** — el freeze ya se deployó en 2.8b). **No correr sin 3a.19b ni sin 3a.19c.**
- [x] **3a.20b [MANUAL] — M-09 (sweep), movido acá desde 3d.6 y degradado a aserción** Correr `migrate-friendships-to-follows.ts --dry-run --since <ISO del run de M-04>`. **Resultado esperado: `toCreate` vacío** — con el freeze en M-03b la ventana de escrituras sobre `friendships` es vacía por construcción, así que esto es una **aserción de que nadie escribió salteando rules**, no un paso de migración. Registrar el resultado en el reporte de apply.
...
- [x] **3b.8 [MANUAL]** Release del build con 3b — coordinar con 3a.21 (deploy de CFs), mismo release, ventana mínima entre ambos.
...
- [x] **3d.1 [GATE]** Confirmar (grep) cero referencias a `Friendship`/`friendshipRepositoryProvider`/`friendshipByPairProvider` fuera de los propios archivos a borrar.
  > **El gate FALLÓ en la primera corrida y destapó un bug real que PR3c se había comido.** Quedaban TRES consumidores vivos en `lib/` de `pendingRequestCountProvider` (la versión `friendships`): `profile_cuenta_section.dart:34`, `notification_history_screen.dart:50` y `notification_history_providers.dart:49` — o sea **el badge de solicitudes pendientes**, en la sección Cuenta y en el historial de notificaciones.
  > **Por qué importaba y no era sólo un import colgado**: el inbox ya se había migrado a `follows` en 3c.14, pero el badge seguía contando `friendships`. Después del flip esa colección queda congelada y las solicitudes nuevas viven en `follows`, así que **el badge habría marcado 0 para siempre mientras el inbox mostraba pedidos reales**. La peor combinación posible: el usuario nunca se entera de que tiene solicitudes.
  > **Arreglo**: los tres repuntados a `pendingFollowRequestCountProvider` (ya existía desde PR1). **Tres tests nuevos que siembran Firestore de verdad** en vez de overridear el provider — con el provider mockeado, contar una colección u otra da idéntico y el test no probaría nada. Cubren: cuenta las pendientes de `follows`, NO cuenta un residuo legacy en `friendships`, y sólo cuenta las RECIBIDAS (no las que yo mandé).
- [x] **3d.2 [GREEN]** Borrar `lib/features/feed/domain/friendship.dart`, `friendship.freezed.dart`, `friendship.g.dart`, `friendship_status.dart`.
- [x] **3d.3 [GREEN]** Borrar `lib/features/feed/data/friendship_repository.dart`, `lib/features/feed/application/friendship_providers.dart`.
- [x] **3d.4 [GREEN]** Borrar los 5 archivos de test del módulo: `friendship_repository_test.dart` (33 tests), `friendship_repository_get_by_pair_test.dart`, `friendship_test.dart`, `friendship_status_test.dart` y `friendship_providers_test.dart` — todos ya reemplazados por `follow_repository_test.dart` (PR1).
  > **Dos suites NO se borraron, se MIGRARON**, porque probaban contratos que el sucesor conserva: el grupo de forma de `acceptedFriendsProvider` en `stream_providers_test.dart` pasó a `followingProvider` (misma forma exacta: `StreamProvider.family.autoDispose<List<String>>`), y `feed-42` de `feed_gap_test.dart` pasó de `acceptedFriendsOf` a `followingOf`. En este último se **agregó** el caso que el modelo viejo no podía expresar: u5 sigue a u1 sin que u1 lo siga, y no aparece entre los seguidos de u1.
- [x] **3d.5 [GATE]** `flutter analyze` 0 issues (sin errores de compilación por imports colgantes) + `dart format .` + `flutter test` verde (baseline ~4748 menos los 33 retirados, más los nuevos de `follow_repository_test.dart`/widgets).
