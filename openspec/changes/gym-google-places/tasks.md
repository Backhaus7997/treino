...
- [ ] 2.2 RED — `test/features/gyms/domain/gym_source_test.dart` (or existing test file): add failing test for `GymSource.googlePlaces` + `'google-places'` wire (de)serialization round-trip.
- [ ] 2.3 GREEN — `lib/features/gyms/domain/gym_source.dart`: add `googlePlaces` enum value + `'google-places'` JSON mapping.
- [ ] 2.4 `lib/features/gyms/domain/gym_suggestion.dart`: create plain DTO `{placeId, primaryText, secondaryText}` (NOT freezed).
- [ ] 2.5 RED — `test/features/gyms/domain/gym_suggestion_test.dart`: failing test constructing/comparing `GymSuggestion` instances (equality/fields).
- [ ] 2.6 GREEN: confirm 2.4 satisfies 2.5.
- [ ] 2.7 VERIFY-AT-APPLY: confirm current Places API (New) Autocomplete endpoint/body/field-mask tokens (`POST /v1/places:autocomplete`, `{input, sessionToken, locationBias:{circle:{center:{latitude,longitude},radius}}}`, `suggestions.placePrediction.(placeId,text,structuredFormat)`) against live Google docs before coding 2.9.
- [ ] 2.8 RED — `test/features/gyms/data/places_autocomplete_service_test.dart`: failing tests for request shape (bias present when location available, omitted when not) and response parsing into `List<GymSuggestion>`, using injected mock `http.Client`.
- [ ] 2.9 GREEN — `lib/features/gyms/data/places_autocomplete_service.dart`: implement Autocomplete (New) REST call via `http`, client key (from `--dart-define`) + session token + optional `locationBias` from geolocator, 300ms debounce input contract, parses response per 2.7.
- [ ] 2.10 RED — `test/features/gyms/data/resolve_gym_place_service_test.dart`: failing test for thin `httpsCallable('resolveGymPlace')` wrapper, region `southamerica-east1`, mirroring `account_deletion_service.dart` pattern.
- [ ] 2.11 GREEN — `lib/features/gyms/data/resolve_gym_place_service.dart`: implement wrapper per 2.10.
- [ ] 2.12 RED — `test/features/gyms/application/places_providers_test.dart`: failing tests for debounced `placesSuggestionsProvider(query)` (`FutureProvider.autoDispose.family`, empty query → `[]`) and `selectGymActionProvider` (calls resolve service then `UserRepository.update({gymId})`, exposes loading/error).
- [ ] 2.13 GREEN — `lib/features/gyms/application/places_providers.dart`: implement both providers per 2.12, wiring `cloudFunctionsProvider` region `southamerica-east1`.
- [ ] 2.14 `flutter pub run build_runner build --delete-conflicting-outputs` — regenerate `.g.dart`/`.freezed.dart` for `GymSource` change (2.3) if code-generated; confirm `gym_suggestion.dart` stays hand-written (not freezed, per design).
- [ ] 2.15 Quality gate (Slice 2): `flutter analyze` 0 issues, `dart format .` clean, `flutter test test/features/gyms/` green.
