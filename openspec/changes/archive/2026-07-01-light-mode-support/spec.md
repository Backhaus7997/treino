...
New ARB keys SHALL be added to `lib/l10n/intl_es_AR.arb` (primary) and `intl_en.arb` (scaffold). Required keys: `appearanceSettingsTitle`, `themeModeSystem`, `themeModeLight`, `themeModeDark`, `appearanceSubtitle`. ARB codegen SHALL complete without warnings.

#### SCENARIO-829: ARB codegen runs clean

- GIVEN all 5 keys are present in both ARB files
...
- WHEN `AppLocalizations.of(context).appearanceSettingsTitle` is read
...
The base body silhouette PNGs (`bodyfront.png`, `bodyback.png`) SHALL be wrapped in `ColorFiltered(colorFilter: ColorFilter.mode(palette.textPrimary, BlendMode.srcIn), ...)` so the silhouette color tracks `textPrimary` (dark on light bg, light on dark bg). Mask overlays SHALL continue to tint with `palette.accent`.
...
- THEN the base silhouette is visibly dark against the light background

#### SCENARIO-832: Silhouette visible on dark background

- GIVEN `ThemeMode.dark` is active and the insights body map is displayed
- WHEN the screen renders
- THEN the base silhouette is visibly light against the dark background
