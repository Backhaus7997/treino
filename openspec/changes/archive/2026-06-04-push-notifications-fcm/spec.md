# Spec: push-notifications-fcm

**Change**: push-notifications-fcm
**Owner**: Backhaus (Dev C)
**Date**: 2026-06-03
**Phase**: Fase 6 Etapa 2
**Artifact store**: hybrid (file + Engram `sdd/push-notifications-fcm/spec`)
**Proposal ref**: `openspec/changes/push-notifications-fcm/proposal.md`
**Scenario range**: SCENARIO-619..693

---

## Overview

Four new Cloud Functions and a Flutter notification feature area deliver FCM push notifications across chat, agenda, trainer_links, and reviews. This is an entirely new capability area — no existing specs to delta against. All requirements are NEW. The system remains additive-only: users who deny the permission prompt experience no behaviour change.

Delivered in 2 chained PRs: PR#1 (CF layer, `southamerica-east1`) and PR#2 (Flutter layer). A third PR (notification preferences UI) is deferred and out of v1 scope.
...
### REQ-PN-DATA-001 — fcm_tokens Array Field on users/{uid}

The system MUST store FCM tokens as an array field `fcm_tokens: string[]` on the existing `users/{uid}` Firestore document. No new collections or sub-collections MUST be created for token storage.

#### SCENARIO-619: Token field absent on new user document
- **Given** a freshly created `users/{uid}` document with no `fcm_tokens` field
- **When** `FcmTokenRepository.saveToken(uid, 'tok-1')` is called
- **Then** `users/{uid}.fcm_tokens` equals `['tok-1']`
...
- **Given** `users/{uid}.fcm_tokens == ['tok-1']`
- **When** `FcmTokenRepository.saveToken(uid, 'tok-1')` is called again
- **Then** `users/{uid}.fcm_tokens` still equals `['tok-1']` (no duplicate)
- **Test target**: `test/features/notifications/data/fcm_token_repository_test.dart`
- **REQ**: REQ-PN-DATA-001
...
- **Given** `users/{uid}.fcm_tokens == ['tok-phone']`
- **When** `FcmTokenRepository.saveToken(uid, 'tok-tablet')` is called
- **Then** `users/{uid}.fcm_tokens` equals `['tok-phone', 'tok-tablet']`
- **Test target**: `test/features/notifications/data/fcm_token_repository_test.dart`
- **REQ**: REQ-PN-DATA-002
...
- **Given** `users/{uid}.fcm_tokens == ['tok-1', 'tok-2']`
- **When** `FcmTokenRepository.removeToken(uid, 'tok-1')` is called
- **Then** `users/{uid}.fcm_tokens` equals `['tok-2']`
- **Test target**: `test/features/notifications/data/fcm_token_repository_test.dart`
- **REQ**: REQ-PN-DATA-003

#### SCENARIO-623: Removing absent token is a no-op
- **Given** `users/{uid}.fcm_tokens == ['tok-2']`
- **When** `FcmTokenRepository.removeToken(uid, 'tok-999')` is called
- **Then** no error is thrown and `users/{uid}.fcm_tokens` still equals `['tok-2']`
- **Test target**: `test/features/notifications/data/fcm_token_repository_test.dart`
- **REQ**: REQ-PN-DATA-003
...
The `firestore.rules` file MUST NOT be modified by this change. The existing owner-write rule on `users/{uid}` already permits writing `fcm_tokens` as a new array field. This MUST be verified and treated as a binding constraint.

#### SCENARIO-624: Authenticated user can write fcm_tokens under existing rules
- **Given** the current `firestore.rules` unchanged and an authenticated user writing to their own `users/{uid}` document
- **When** a write updating `fcm_tokens` via `arrayUnion` is attempted
...
The project MUST include a shared helper `functions/src/notifications/send-fcm.ts` that accepts `{ uids: string[], notification: { title: string, body: string }, data: Record<string, string> }`. It MUST read `fcm_tokens` per uid via `Promise.all`, call `admin.messaging().sendEachForMulticast({ tokens, notification, data })`, inspect per-token `BatchResponse` errors, and call `arrayRemove` on `users/{uid}.fcm_tokens` for any token returning `messaging/registration-token-not-registered` or `messaging/invalid-registration-token`. Recipients with an empty or absent `fcm_tokens` array MUST be skipped silently with a log line.
...
- **Then** `arrayRemove('tok-stale')` is called on `users/{uid}.fcm_tokens`
...
- **When** `sendFcm` inspects the `BatchResponse`
- **Then** `arrayRemove('tok-invalid')` is called on `users/{uid}.fcm_tokens`
- **Test target**: `functions/src/__tests__/send-fcm.test.ts`
- **REQ**: REQ-PN-CF-001

#### SCENARIO-628: sendFcm skips uid with empty fcm_tokens array
- **Given** a uid whose `users/{uid}.fcm_tokens` is `[]` or absent
...
- **Test target**: `test/features/notifications/application/fcm_service_test.dart`
...
- **Test target**: `test/features/notifications/presentation/permission_prompt_test.dart`
- **REQ**: REQ-PN-PERM-001
...
## Additional Scenarios — Edge Cases and Smoke
...
#### SCENARIO-681: notifyOnReview trainer with empty fcm_tokens — skip silently
- **Given** a review is created for a trainer whose `users/{trainerId}.fcm_tokens` is `[]`
...
- **And** no exception is thrown
- **Test target**: `test/features/notifications/application/notification_handler_test.dart`
- **REQ**: REQ-PN-HANDLER-001, REQ-PN-HANDLER-002
...
|---|---|---|---|
| REQ-PN-DATA-001 | fcm_tokens array field on users/{uid} | SCENARIO-619, 620 | PR#2 |
| REQ-PN-DATA-002 | saveToken uses arrayUnion | SCENARIO-621 | PR#2 |
| REQ-PN-DATA-003 | removeToken uses arrayRemove | SCENARIO-622, 623 | PR#2 |
| REQ-PN-DATA-004 | No firestore.rules change required | SCENARIO-624 | PR#2 |
| REQ-PN-CF-001 | send-fcm shared helper | SCENARIO-625, 626, 627, 628, 677 | PR#1 |
| REQ-PN-CF-002 | notifyOnChatMessage trigger | SCENARIO-629, 630, 631, 680 | PR#1 |
| REQ-PN-CF-003 | notifyOnAppointment trigger | SCENARIO-632, 633, 634, 635, 636, 684 | PR#1 |
| REQ-PN-CF-004 | notifyOnLinkChange trigger | SCENARIO-637, 638, 639, 640, 641 | PR#1 |
| REQ-PN-CF-005 | notifyOnReview trigger | SCENARIO-642, 681 | PR#1 |
| REQ-PN-CF-006 | All CFs exported from index.ts | SCENARIO-643 | PR#1 |
| REQ-PN-CLIENT-001 | firebase_messaging dep, no flutter_local_notifications | SCENARIO-644 | PR#2 |
| REQ-PN-CLIENT-002 | FcmService.init saves token + watches refresh | SCENARIO-645, 646, 647, 678 | PR#2 |
| REQ-PN-CLIENT-003 | FcmService.dispose removes token on logout | SCENARIO-648, 649, 679 | PR#2 |
| REQ-PN-CLIENT-004 | Riverpod provider wires FcmService to auth state | SCENARIO-650, 651, 683 | PR#2 |
| REQ-PN-HANDLER-001 | Foreground message shows SnackBar + deep link | SCENARIO-652, 653, 654, 682 | PR#2 |
| REQ-PN-HANDLER-002 | Background tap navigates via deep link | SCENARIO-655, 656 | PR#2 |
| REQ-PN-HANDLER-003 | Cold-start tap navigates after router ready | SCENARIO-657, 658 | PR#2 |
| REQ-PN-PERM-001 | Permission requested post-onboarding only | SCENARIO-659, 660, 661 | PR#2 |
| REQ-PN-PERM-002 | Permission copy + graceful denial | SCENARIO-662, 663 | PR#2 |
| REQ-PN-CX-001 | iOS UIBackgroundModes in Info.plist | SCENARIO-664 | PR#1 (Info.plist) |
| REQ-PN-CX-002 | No flutter_local_notifications | SCENARIO-665 | PR#2 |
| REQ-PN-CX-003 | Body length ≤ 256, chat truncated at 100 | SCENARIO-666 | PR#1 |
| REQ-PN-CX-004 | Existing collections only, no new indexes | SCENARIO-667 | both PRs |
| REQ-PN-CX-005 | storage.rules unchanged | SCENARIO-668 | both PRs |
| REQ-PN-CX-006 | es-AR copy with i18n markers | SCENARIO-669 | both PRs |
| REQ-PN-CX-007 | Zero HEX literals / zero PhosphorIcons direct | SCENARIO-670 | PR#2 |
| REQ-PN-CX-008 | Strict TDD | SCENARIO-671 | both PRs |
| REQ-PN-CX-009 | Conventional commits, no AI attribution | SCENARIO-672 | both PRs |
| REQ-PN-CX-010 | LOC budget per PR | SCENARIO-673, 674 | both PRs |
| REQ-PN-CX-011 | APNs auth key as manual prerequisite | SCENARIO-675 | out-of-band |
| REQ-PN-CX-012 | CF emulator tests, FCM dispatch mocked | SCENARIO-676 | PR#1 |
...
|---|---|---|
| PR#1 — CF layer | REQ-PN-CF-001..006, REQ-PN-CX-001, REQ-PN-CX-003, REQ-PN-CX-006, REQ-PN-CX-009, REQ-PN-CX-010, REQ-PN-CX-012 | ~350 |
| PR#2 — Flutter layer | REQ-PN-DATA-001..004, REQ-PN-CLIENT-001..004, REQ-PN-HANDLER-001..003, REQ-PN-PERM-001..002, REQ-PN-CX-002, REQ-PN-CX-004, REQ-PN-CX-005, REQ-PN-CX-006, REQ-PN-CX-007, REQ-PN-CX-008, REQ-PN-CX-009, REQ-PN-CX-010 | ~400 |
| Out-of-band | REQ-PN-CX-011 | — |

---

## Open Questions for Design

1. **Cold-start router gate implementation detail.** REQ-PN-HANDLER-003 specifies a post-first-frame callback as the gate, but the exact integration point (`app.dart initState`, `TreinoApp.build`, or a root `NavigatorObserver`) is left to the design phase to lock.
2. **`profile_setup_completed` flag source.** The spec assumes this flag exists as a readable field or provider. Design must confirm which Riverpod provider or Firestore field exposes it and whether it requires a new selector or can use an existing one.
3. **`FcmService` singleton vs `ref.listen` pattern.** Design must choose between a singleton service class and a pure Riverpod `ref.listen` in a top-level provider — both satisfy the spec but have different testability implications.
4. **Foreground handler attachment point.** The spec says "attach in `app.dart initState`". Design must confirm whether `TreinoApp` or a child widget is the right scope, and whether a `ConsumerStatefulWidget` or `ref.listen` approach is cleaner for stream lifetime management.

---

## Hard Constraints

These are enforceable requirements that MUST be met — no exceptions without a separate RFC:
...
- File: `openspec/changes/push-notifications-fcm/spec.md`
- Engram: `sdd/push-notifications-fcm/spec`
- Proposal: `openspec/changes/push-notifications-fcm/proposal.md` + Engram `sdd/push-notifications-fcm/proposal`
- Exploration: `openspec/changes/push-notifications-fcm/explore.md` + Engram `sdd/push-notifications-fcm/explore` (#137)
