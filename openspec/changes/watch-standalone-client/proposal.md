...
| 2 | Auth en el reloj | El reloj tiene **credencial propia**, no prestada: una Cloud Function (`mintWatchCredential`) mintea un custom token para el uid del caller, el teléfono se lo pasa por WatchConnectivity, y el reloj lo canjea por su **propio** idToken + refreshToken. Renueva solo por HTTP contra `securetoken.googleapis.com`, sin SDK de Firebase. | Google Sign-In no tiene SDK de watchOS. **CORREGIDO en F1**: el diseño original —"el teléfono entrega SU refresh token"— es imposible. `User.refreshToken` de firebase_auth es vacío en nativo, documentado textual en el platform interface: *"empty string for native platforms (android, iOS & macOS)"*. La salida por custom token es además mejor: la credencial del reloj es revocable por separado y el teléfono nunca expone la suya. **Propiedad de seguridad load-bearing**: la función mintea SOLO para `request.auth.uid` y no acepta uid por parámetro. |
