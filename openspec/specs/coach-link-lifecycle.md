# Spec: coach-link-lifecycle

**Domain**: Coach Link Lifecycle  
**REQ namespace**: `REQ-COACH-LINK-NNN`  
**Status**: Current (Fase 5 · Tech Debt)

---

## Overview

`coach-link-lifecycle` defines the full lifecycle of a `TrainerLink` — the bidirectional relationship between an athlete and a trainer. This includes:
- Link state transitions (request → accept → active / decline / cancel / terminate)
- Privacy controls (athlete can share workout history with trainer)
- Firestore data contract and security rules

---

## Requirements

| ID | Name | Strength | Source |
|----|------|----------|--------|
| REQ-COACH-LINK-001 | `TrainerLink.sharedWithTrainer` field — model contract | MUST | Fase 5 |
| REQ-COACH-LINK-002 | `TrainerLink.sharedWithTrainer` default value | MUST | Fase 5 |
| REQ-COACH-LINK-003 | `TrainerLinkRepository.setSharedWithTrainer` — update contract | MUST | Fase 5 |
| REQ-COACH-LINK-004 | `setSharedWithTrainer` field isolation — no side effects | MUST | Fase 5 |
| REQ-COACH-LINK-005 | `setSharedWithTrainer` missing link throws | MUST | Fase 5 |
| REQ-COACH-LINK-006 | `setSharedWithTrainer` idempotent write | MUST | Fase 5 |
| REQ-COACH-LINK-007 | Toggle visible only when `link.status == active` | MUST | Fase 5 |
| REQ-COACH-LINK-008 | Toggle reflects `link.sharedWithTrainer` | MUST | Fase 5 |
| REQ-COACH-LINK-009 | Enabling toggle shows confirmation dialog | MUST | Fase 5 |
| REQ-COACH-LINK-010 | Confirming dialog calls repo and invalidates provider | MUST | Fase 5 |
| REQ-COACH-LINK-011 | Disabling toggle skips dialog and calls repo directly | MUST | Fase 5 |
| REQ-COACH-LINK-012 | Firestore rule — athlete can flip `sharedWithTrainer` | MUST | Fase 5 |
| REQ-COACH-LINK-013 | Firestore rule — trainer CANNOT flip `sharedWithTrainer` | MUST | Fase 5 |
| REQ-COACH-LINK-014 | Firestore rule — non-member CANNOT update the document | MUST | Fase 5 |
...
1. **No `updatedAt`**: `setSharedWithTrainer` MUST NOT write `updatedAt`. This matches the
   convention of all other `TrainerLinkRepository` update methods.
...
3. **Trainer cannot see the toggle**: `_LinkStateCard` is private to `athlete_coach_view.dart`.
   `trainer_coach_view.dart` does NOT render it.
...
5. **Etapa 6 dependency**: `sharedWithTrainer` is a privacy gate only. Etapa 6 (Agenda + PF) will
   add the query filter on `sessions/{athleteId}/*` using this field. This change MUST NOT add
   any Etapa 6 behavior — the gate is ready for consumption.
...
**Spec Version**: 1.0 (Fase 5 · Tech Debt)  
