# Post-Workout Summary — UI Layer Specification

**Domain**: Workout Session Presentation
**Change**: `post-workout-summary` (Fase 4 · Etapa 3)
**Status**: ACTIVE (source of truth)
**Last updated**: 2026-05-19
**Merged**: PR #39 (squash commit c23c80c); merged to main

---

## Purpose

Define the post-workout summary screen presentation layer: the `PostWorkoutSummaryScreen` widget, the `PostWorkoutNotifier` (share controller), session summary data provider, and router wiring for the route `/workout/session-summary/:sessionId`. This spec is the authoritative reference for all summary presentation components and becomes the canonical spec after the change is archived.
...
|----|------|----------|--------|
| REQ-PWS-001 | PostWorkoutSummaryScreen widget contract | MUST | ✅ IMPLEMENTED |
| REQ-PWS-002 | Screen loads session on boot | MUST | ✅ IMPLEMENTED |
| REQ-PWS-003 | Not-found state when session absent | MUST | ✅ IMPLEMENTED |
| REQ-PWS-004 | Error state on getById failure | MUST | ✅ IMPLEMENTED |
| REQ-PWS-005 | Header reflects completion status | MUST | ✅ IMPLEMENTED |
| REQ-PWS-006 | 2×2 stat grid with session metrics | MUST | ✅ IMPLEMENTED |
| REQ-PWS-007 | PRs section rendered as stub | MUST | ✅ IMPLEMENTED |
| REQ-PWS-008 | Emoji mood row — visual only | MUST | ✅ IMPLEMENTED |
| REQ-PWS-009 | LISTO button navigates to /workout | MUST | ✅ IMPLEMENTED |
| REQ-PWS-010 | COMPARTIR button triggers shareWorkout | MUST | ✅ IMPLEMENTED |
| REQ-PWS-011 | shareWorkout creates Post on success | MUST | ✅ IMPLEMENTED |
| REQ-PWS-012 | shareWorkout shows error SnackBar on failure | MUST | ✅ IMPLEMENTED |
| REQ-PWS-014 | Router replaces stub with PostWorkoutSummaryScreen | MUST | ✅ IMPLEMENTED |
