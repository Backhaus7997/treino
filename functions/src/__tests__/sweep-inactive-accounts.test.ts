...
// Las dos puertas que el grafo de este test alcanza sin llamarlas nunca.
//
// No es ceremonia: el barrido importa `runDeleteAccount`, y con él la cascada
// entera — incluida `cascade/storage.ts`, que pide `getStorage`. Sin estos dos
// dobles esos módulos resuelven contra el SDK REAL con un `firebase-admin/app`
// de mentira al lado, que es exactamente el agujero que vigila
// `firebase-admin-mock-surface.test.ts`. Lo encontró él, no yo.
