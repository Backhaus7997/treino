// Harness de evidencia visual de la sección "Ajustes" del Coach Hub web
// (EVIDENCE-HARNESS).
//
// Captura 4 screenshots (goldens) de la pantalla de ajustes (`/ajustes`)
// montada DENTRO del shell real (CoachHubScaffold) con proveedores falsos
// POBLADOS (perfil de PF completo + vínculos activos + prefs de
// notificaciones) para que el screenshot muestre data real, no vacío/error.
// Estos PNGs sirven como línea base BEFORE/AFTER para validar regresiones
// visuales de la Fase 12.
//
// Mismo patrón que test/evidence/coach_hub_pagos_evidence_test.dart — ver ese
// archivo para el detalle de por qué se cargan las fuentes así.
//
...
//     test/evidence/coach_hub_ajustes_evidence_test.dart
//
// Los PNGs se guardan en:
//   docs/web-trainer/evidence/fase-12/<EVIDENCE_DIR>/
//
...
import 'package:treino/features/coach_hub/presentation/sections/ajustes/tabs/notificaciones_prefs.dart'
    show NotifPrefs, webNotificationPreferencesProvider;
...
// ──────────────────────────────────────────────────────────────────────────────
// Factories — datos fake poblados: perfil de PF completo + vínculos activos.
// ──────────────────────────────────────────────────────────────────────────────
...
TrainerLink _link(String id, String athleteId) => TrainerLink(
      id: id,
...
// ──────────────────────────────────────────────────────────────────────────────
// Helper: monta el shell real (CoachHubScaffold) con la pantalla de Ajustes
// REAL en `/ajustes`, GoRouter + proveedores falsos POBLADOS.
// ──────────────────────────────────────────────────────────────────────────────
Future<void> _pumpAjustes(
  WidgetTester tester, {
  ...
  // ── Datos fake poblados ──────────────────────────────────────────────────
  // 3 vínculos activos (Zona Peligrosa + Facturación muestran conteo real).
  final links = [
    _link('link1', 'a1'),
    _link('link2', 'a2'),
    _link('link3', 'a3'),
  ...
  final container = ProviderContainer(overrides: [
    ...
    webNotificationPreferencesProvider.overrideWith(
      (ref) => Stream.value(NotifPrefs.fromFirestore(null)),
    ),
  ]);
  addTearDown(container.dispose);

  // Warm de providers que el shell lee en build.
  ...
  // GoRouter: ShellRoute con CoachHubScaffold + rutas reales de Ajustes
  // (`/ajustes`) + el resto de rutas del sidebar como stub liviano, necesarias
  // para que el sidebar resuelva GoRouterState.of(context) (mismo patrón que
  // coach_hub_pagos_evidence_test.dart). `/ajustes` se excluye del loop
  // porque ya la aporta `ajustesRoutes`.
  final otherPaths =
      sidebarRegistry.map((i) => i.route).toSet().where((p) => p != '/ajustes');
  ...
  await tester.pumpWidget(
    UncontrolledProviderScope(
      ...
      child: MaterialApp.router(
        ...
        // de Ajustes quedarían vacíos (l10n congelado en español).
  ...
  // Guard endurecido: título real exacto + la tab Cuenta real montada.
  expect(
    find.text('CONFIGURACIÓN').evaluate().isNotEmpty,
    isTrue,
    reason: 'Título "CONFIGURACIÓN" ausente — la screen real de Ajustes no '
        'se montó (regresión al placeholder o l10n español ausente).',
  );
  expect(
    find.text('INFORMACIÓN PERSONAL').evaluate().isNotEmpty,
    isTrue,
    reason: 'Card "INFORMACIÓN PERSONAL" ausente — la tab Cuenta real no se '
        'montó (regresión o userProfileProvider no resolvió el perfil fake).',
  );
}

// Cuerpo de sección stub para las rutas del sidebar que no son /ajustes.
...
// ──────────────────────────────────────────────────────────────────────────────
// Comparador de goldens que escribe en docs/web-trainer/evidence/fase-12/<dir>/
// ──────────────────────────────────────────────────────────────────────────────
class _EvidenceComparator extends LocalFileComparator {
  ...
  Future<bool> compare(Uint8List imageBytes, Uri golden) async {
    const basePath = 'docs/web-trainer/evidence/fase-12/$_evidenceDir';
  ...
  Uri getTestUri(Uri key, int? version) {
    return Uri.file('docs/web-trainer/evidence/fase-12/$_evidenceDir/'
...
void main() {
  ...
  group('Coach Hub Ajustes — evidencia visual fase-12/$_evidenceDir', () {
        ...
        testWidgets(
          ...
          (tester) async {
            ...
            await expectLater(
              ...
              matchesGoldenFile('ajustes_${modeLabel}_$sizeLabel.png'),
            );
          },
        );
      }
