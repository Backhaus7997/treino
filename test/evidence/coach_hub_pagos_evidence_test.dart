// Harness de evidencia visual de la sección "Pagos" del Coach Hub web
// (EVIDENCE-HARNESS).
//
// Captura 4 screenshots (goldens) de la pantalla de pagos (`/pagos`) montada
// DENTRO del shell real (CoachHubScaffold) con proveedores falsos POBLADOS
// (pagos vencidos/por vencer/pagados + perfiles) para que el screenshot
// muestre data real, no vacío/error. Estos PNGs sirven como línea base
// BEFORE/AFTER para validar regresiones visuales de la Fase 9.
//
...
//     test/evidence/coach_hub_pagos_evidence_test.dart
//
// Los PNGs se guardan en:
//   docs/web-trainer/evidence/fase-9/<EVIDENCE_DIR>/
//
...
// ──────────────────────────────────────────────────────────────────────────────
// Factories — datos fake poblados: ≥2 vencidos + ≥2 por vencer + ≥2 pagados.
// ──────────────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────────
// Helper: monta el shell real (CoachHubScaffold) con la pantalla de pagos
// REAL en `/pagos`, GoRouter + proveedores falsos POBLADOS.
// ──────────────────────────────────────────────────────────────────────────────
Future<void> _pumpPagos(
  WidgetTester tester, {
  ...
  // ── Datos fake poblados ──────────────────────────────────────────────────
  // ≥2 vencidos (pending, dueAt pasado) + ≥2 por vencer (pending, dueAt
  // futuro) + ≥2 pagados (paid, paidAt en el mes actual).
  ...
  final payments = [
    ...
    _payment(
      ...
      athleteId: 'a2',
      amountArs: 22000,
      concept: 'Plan premium', // i18n
      status: PaymentStatus.pending,
      createdAt: now.subtract(const Duration(days: 20)),
      dueAt: now.subtract(const Duration(days: 12)),
    ),
    _payment(
      id: 'pay3',
      athleteId: 'a3',
    ...
    _payment(
      ...
      paidAt: now.subtract(const Duration(hours: 26)),
    ),
  ];
  ...
  // GoRouter: ShellRoute con CoachHubScaffold + rutas reales de Pagos
  // (`/pagos`) + el resto de rutas del sidebar como stub liviano, necesarias
  // para que el sidebar resuelva GoRouterState.of(context) (mismo patrón que
  // coach_hub_solicitudes_evidence_test.dart). `/pagos` se excluye del loop
  // porque ya la aporta `pagosRoutes`.
  final otherPaths =
      sidebarRegistry.map((i) => i.route).toSet().where((p) => p != '/pagos');
  ...
  await tester.pumpWidget(
    UncontrolledProviderScope(
      ...
      child: MaterialApp.router(
        ...
        // labels de Pagos quedarían vacíos (l10n congelado en español).
  ...
  // Guard endurecido: título real exacto + nombre de un alumno fake —
  // confirma que la screen real de Pagos está montada Y que la data poblada
  // (pagos + perfiles) llegó hasta el render, no solo el título.
  expect(
    find.text('PAGOS').evaluate().isNotEmpty,
    isTrue,
    reason: 'Título "PAGOS" ausente — la screen real de Pagos no se montó '
        '(regresión al placeholder o l10n español ausente).',
  );
  expect(
    find.textContaining('Camila Vega').evaluate().isNotEmpty,
    isTrue,
    reason: 'Nombre del alumno fake ("Camila Vega") ausente — '
        'trainerPaymentsProvider o userPublicProfilesBatchProvider no están '
        'resolviendo la data poblada del harness.',
  );
}

// Cuerpo de sección stub para las rutas del sidebar que no son /pagos.
...
// ──────────────────────────────────────────────────────────────────────────────
// Comparador de goldens que escribe en docs/web-trainer/evidence/fase-9/<dir>/
// ──────────────────────────────────────────────────────────────────────────────
class _EvidenceComparator extends LocalFileComparator {
  ...
  Future<bool> compare(Uint8List imageBytes, Uri golden) async {
    const basePath = 'docs/web-trainer/evidence/fase-9/$_evidenceDir';
  ...
  Uri getTestUri(Uri key, int? version) {
    return Uri.file('docs/web-trainer/evidence/fase-9/$_evidenceDir/'
...
void main() {
  ...
  group('Coach Hub Pagos — evidencia visual fase-9/$_evidenceDir', () {
        ...
        testWidgets(
          ...
          (tester) async {
            ...
            await expectLater(
              ...
              matchesGoldenFile('pagos_${modeLabel}_$sizeLabel.png'),
            );
          },
        );
      }
