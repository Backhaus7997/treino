...
| 6 | ProfileSetup flow + Storage avatars ✅ | `72e1947` (#4) | Storage bucket creado (**`us-east1`** — esta celda decia `southamerica-east1` y era FALSO; medido contra la API de GCS el 2026-09-15, y esa copia rompio el deploy de los triggers de Storage) | Multi-step de 4 pantallas: username, gym, experiencia/género, peso/altura, avatar. `firebase_storage`, `image_picker`. Redirect post-signup cuando `UserProfile` está incompleto. | B |
...
| 5 | **Excel polish + aliases dinámicos** ✅ — Excel template polish + `addAlias` CF end-to-end | `2bc5b14` (#136 catalog import 25→415 + es-AR técnica + dedup generics) + `3319a33` (#142 PR#1 template polish) + `6152e23` (#143 PR#2a addAlias CF + parity tests) + `63d08f9` (#144 PR#2b client wire) + housekeeping `bc8ff25` (`docs/setup/firebase-hosting-callable-functions.md`) | Cloud Function `addAlias` deployada en `southamerica-east1` con Compute SA `roles/run.invoker` (suficiente para server-to-server + mobile). **Browser CORS gap**: org policy `Domain Restricted Sharing` impide `allUsers` invoker → Coach Hub web no puede invocar la CF sin Firebase Hosting rewrites (deuda documentada, no bloquea mobile/server). | 3 PRs chained bajo Strict TDD (SDD `coach-excel-polish`): **PR#1 #142** template polish (~180 LOC) — `kColumnWidthsDay`/`kColumnWidthsPlan` constantes + hoja `Instrucciones` con guía de columnas, valores válidos de `Nivel`, ejemplo pre-cargado. Validation dropdown nativo no soportado por `excel: ^4.0.6` → reemplazado por texto referencial (scope tradeoff aceptado). **PR#2a #143** addAlias CF v2 en `southamerica-east1` (~310 LOC) — `runAddAlias(app, callerId, exerciseId, alias)` pure handler con auth gate + role gate (`users/{callerId}.role == 'trainer'`) + exercise existence check + dedup + `arrayUnion(normalized)`. TS `normalize()` literal port char-by-char del Dart `normalize()` con comentarios `// NORMALIZE-PARITY: see ADR-CXP-006` en ambos lados (R1 load-bearing). 14 jest emulator tests cubriendo auth/role/normalize/dedup/idempotency/accent parity. **PR#2b #144** client wire (~160 LOC) — `cloudFunctionsProvider` Riverpod provider + `unawaited(_addAlias(picked.id, rowName))` fire-and-forget AFTER setState BEFORE next await en `_pickExerciseFor` (ADR-CXP-009), errores silenciosos con `debugPrint`. 4 widget tests con `ProviderScope.override` del cloudFunctions provider. **Progreso 2026-06-08**: PR#136 expandió el catálogo dev de 25 → 415 ejercicios (free-exercise-db import + dedup de generics + hand-written es-AR `techniqueInstructions` para ~80 ejercicios core + nuevos muscleGroup `cardio` y `fullbody`). **⚠️ Superado por PR#209 (Catálogo de ejercicios desde el Drive)**: el catálogo real hoy son **793 ejercicios** con campos `id/name/muscleGroup/equipoES/fileId/filename` — todos con video, sin campo de imagen; los ids de PR#136 ya no existen (los 25 PNGs de `assets/exercises/` pertenecen al catálogo viejo). Fuente de verdad: `docs/video-catalog-audit/NUEVO-catalogo.json`. Verify PASS-WITH-DEVIATIONS: 27/27 REQs + 21/21 SCENARIOs (727-747) + 12/12 ADRs honored, 0 CRITICAL, 4 WARNINGs ops/docs, 2 SUGGESTIONs cosmetic. Flutter 1691/1691 + jest 14/14. Archive en `openspec/changes/archive/2026-06-09-coach-excel-polish/` + main spec `openspec/specs/coach-excel-polish/spec.md`. | B |
| 6 | **App Check + Crashlytics + Analytics** ⏳ — telemetría básica, **código cerrado / consola pendiente** | `feat/telemetry-app-check-crashlytics-analytics` + cierre en `feat/telemetria-cierre-etapa6` | App Check habilitado con App Attest (iOS) + Play Integrity (Android); Crashlytics activado; Analytics conectado a BigQuery export | `firebase_app_check`, `firebase_crashlytics`, `firebase_analytics` packages. App Check rules en Firestore para gate de servicios. Crashlytics captura crashes + non-fatals. Analytics eventos: `session_start`, `routine_started`, `routine_finished`, `plan_assigned`, `link_requested`, `link_accepted`, `chat_message_sent`, `appointment_created`. Dashboard custom en BigQuery o sheets export. **Esta fila decía ⏳ mientras la línea 20 de este mismo archivo daba la etapa por ✅** — el resumen tenía razón sobre casi todo, pero no sobre todo, y auditar el código encontró dos agujeros reales que el ⏳ no nombraba: (1) `appointment_created` estaba declarado en las 3 implementaciones de `AnalyticsService` y **no lo llamaba nadie** — evento en el contrato, cero en los datos; ahora sale de los dos creadores vivos (`createByTrainer` y `createRecurringByTrainer`, con `occurrences` para que una serie de N cuente N y no 1). El docstring apuntaba a `AppointmentRepository.book`, muerto desde #831. (2) Crashlytics tomaba **sólo fatales**: las 3 vías de `main.dart` van con `fatal: true` y no había un solo `fatal: false`. El write cross-feature de contadores de ADR-WRS-10 fallaba a `developer.log`, invisible en producción, dejando el perfil público con números viejos sin que nadie se enterara; ahora sale como non-fatal por `core/telemetry/non_fatal.dart`. **NO se cablearon los ~20 `developer.log` restantes**: varios tragan a propósito (`FcmService.init` documenta que evita ruido de APNS en Crashlytics), así que cada uno necesita criterio propio. **Por qué sigue en ⏳ y no en ✅**: el código de esta etapa está cerrado, pero lo que la etapa PROMETE —que App Check proteja y que Analytics exporte— depende de dos cosas que viven en la consola y hoy no están: el enforcement y el export a BigQuery. Marcarla ✅ sería exactamente la advertencia falsa de AGENTS.md §11.1: un cartel tranquilizador sobre una protección que no está encendida. Vuelve a ✅ cuando eso se verifique. | A |
...
| Fase 6 (Producto-ready: trainer UI + push + agenda recurrente + telemetría + reviews + i18n + self-routines + chat-media + insights polish + bidirectional data channel + exercise notes) | ✅ **CERRADA (mobile)** 2026-06-12 core + extensiones + polish — Etapa 6 telemetría ✅ (Crashlytics #108, Analytics #109, App Check #110); Etapa 7 reviews ✅ 2026-06-02 (`trainer-reviews`); Etapa 4 recurring ✅ 2026-06-04 (trainer-driven eager + cancel-serie); Etapa 2 push notifications ✅ 2026-06-04 (`push-notifications-fcm`, 4 PRs + archive `4551868`); Etapa 1 trainer profile UI ✅ 2026-06-08 (`trainer-profile-onboarding`, 2 PRs #139 #141 + archive `aaf5243`); **Etapa 3 Coach Hub polish ✅ confirmada 2026-06-09 vía audit profundo (PR#134 data + PR#135 CF + PR#138 UI)**; **Etapa 9 self-routines ✅** + supersets en self-create (#137) + **extensión home 2026-06-18** (#191 PR#1 today's routine + #193 PR#2 active marker); **Etapa 5 Excel polish + aliases dinámicos ✅ 2026-06-09** (`coach-excel-polish` SDD: catalog #136 + template #142 + addAlias CF #143 + client wire #144); **Etapa 8 i18n mobile ✅ 2026-06-12** (5 PRs #146/#147/#150/#152/#154, ~250 keys ARB, suite 2052→2328); **Etapa 10 chat-media ✅ 2026-06-19** (`chat-media-messages` SDD, 2 PRs #194 data + #195 UI, foto/video inline en chat PT↔athlete, reemplaza fallback a WhatsApp). **Polish post-build #2 (2026-06-19)**: Insights granular ✅ (#196 — taxonomía colapsada 6→10 grupos alineada al exercise picker, cutoff 2B sin migración de data legacy), chat unread-count ✅ (#197 — cierra el out-of-scope explícito del SDD chat-media), body highlighting ✅ (#198 — masks dinámicas en Insights + Home con intensidad `done/target` clampada [0,1] + fallback 0.6 off-plan, nuevo asset `mask_back_triceps.png`). **Bug operativo descubierto** (2026-06-19): el feed estaba roto desde build #1 porque el index composite `posts (privacy, createdAt)` estaba en `firestore.indexes.json` pero nunca se había deployado a `treino-dev` — fix: `firebase deploy --only firestore:indexes`, follow-up abierto para automatizar via GitHub Action. **Polish post-build #2 (2026-06-29) — bidirectional data channel y notas por ejercicio**: SDD `trainer-athlete-set-logs` ✅ (3 PRs — `1ce1a07` #199 fix wiring de `publicProfileRepository` en `sessionRepositoryProvider` que destrabó las queries cross-user, `019073f` #200 PR#1 foundation + web — relajación de Firestore rule sobre `session_shares` para permitir que el trainer linkeado lea `setLogs/` del athlete + nuevo CF `sync-session-share` con 238 LOC de tests + extracción de widget `session_exercise_block.dart` que renderea exercise/set breakdown reutilizable entre mobile y web, + UI en Coach Hub web Alumno detail → Entrenamientos tab expandible, + 5 ADRs SDD, + 30+ tests, + 20 i18n keys en es-AR/es/en, `49bc1ca` #201 PR#2 mobile — wire en `athlete_detail_screen.dart` para que el PF mobile vea la misma session history granular que ve en web). Cierra recommendation 8.6 del research PT ("Máxima"). SDD `exercise-notes` ✅ (`7ec2af9` #202 — PF agrega notas opcionales por ejercicio en el editor; athlete las ve en el `session_player` durante el entrenamiento, widget `coach_note.dart` reutilizable + integración no-intrusive en `exercise_slot_row.dart` y `session_player_screen.dart`, 5 archivos l10n actualizados, +407 LOC tests, 5 artefactos SDD). **Avatar cropper circular cross-platform ✅** (`c5b0106` #203 — `image_cropper ^9.0.0` + helper centralizado `lib/core/image/avatar_cropper.dart` con UI settings 1:1 lock para Android/iOS/Web, wireado en los 3 surfaces que pickean avatar: profile_edit + profile_setup + cuenta_tab del Coach Hub web; setup nativo `UCropActivity` en AndroidManifest + croppie 2.6.5 desde CDN en web/index.html; refactor del `avatar_web_uploader` a `pickFile + uploadCroppedPath` para meter el cropper entre pick y upload; 3 i18n keys nuevas en es-AR). **Editable day name ✅** (`5871437` #204 — pencil icon al lado del título del día en el editor; tap → `TextField` inline; submit/blur commit; empty/whitespace restaura el default localizado "Día N" sin error; reusa infra existente `_EditableDay.name + isDefaultName` sin nuevos campos de dominio; 5 widget tests + 1 i18n key a11y). **Unlock all days ✅** (`4670607` #205 — A1: drop sequential gate de rutinas periodizadas, `isWeekUnlocked` e `isDayUnlocked` siempre `true`, `isStartable` se reduce a "not already completed"; athletes pueden saltar libremente entre días y semanas sin tener que completar los anteriores; "DÍA BLOQUEADO" / "SEMANA BLOQUEADA" badges desaparecen; nuevo edge case conocido: el home today's routine card sugiere `lastFinished + 1` aún si el athlete saltó). **Coach Hub web i18n deferred a mini-SDD futuro**. **Build #3 ✅ DELIVERED 2026-06-29** (`0.1.0+3`, 80 MB IPA, cero ITMS warnings, bundle de 10 PRs polish post-#2 #196 a #205, Ready to Submit a Internal Beta). | ✅ 2026-06-12 core / 2026-06-29 con extensiones + polish | Proyectada ~2026-06-15 — **3 días adelantada en el core** |
| Fase 7 (Monetización + Lanzamiento beta) | 🔄 **En curso — TestFlight build #3 LIVE 2026-06-29** (`0.1.0+3`, 80 MB IPA, cero ITMS warnings, asignado al grupo Internal Beta tras delivery; 8 testers en uso desde 2026-06-30). **Build #4 candidate (acumulando)**: SDD `exercise-progression` PR#1 mobile #206 (PR#2 web pendiente, hace el equivalente en Coach Hub Web `alumno_detail_screen.dart` reemplazando el placeholder `'Próximamente: evolución por ejercicio...'`). **Builds previos activos en paralelo** (no expirados — fallback si surge regresión en #3): #2 (`0.1.0+2`, 2026-06-18, 8 invites / 7 installs / 17 sessions) + #1 (`0.1.0+1`, 2026-06-17, 8 invites / 9 installs / 67 sessions, 1 ITMS-90683 location flag non-blocking, app name "TREINO Fitness" en App Store Connect porque "TREINO" estaba ocupado, App ID `6781307745`, primary language Spanish Mexico). **Build #3 incluye 10 PRs polish post-#2** (#196 a #205): Insights granular #196 + chat unread #197 + body highlighting + tríceps asset #198 + fix wiring publicProfileRepository #199 + SDD `trainer-athlete-set-logs` PR#1 foundation/web #200 + PR#2 mobile #201 + SDD `exercise-notes` #202 + avatar cropper cross-platform #203 + editable day name #204 + unlock all days #205. **Apple admin gotcha encontrado durante el build #3**: primer `flutter build ipa --release` falló con `error: exportArchive Unable to process request - PLA Update available` + `No signing certificate "iOS Distribution" found`. Causa: Apple actualizó el Program License Agreement y bloquea cert access hasta que el holder acepta el nuevo PLA en https://developer.apple.com/account. Después de aceptar, retry funcionó limpio. Pattern operativo a recordar: si flutter build ipa tira error de signing cert, revisar primero PLA pending antes de tocar configuración de Xcode. **Pre-build #1 polish wave** (todavía vigente): bleed-through fix (#166), exercise name fallback + asset cascade con `assets/muscles/` movement patterns (#176), branded icon TR mint (#178), Info.plist permissions + Export Compliance + iOS 14.0 deployment + aps-environment production (#164), audit batches HIGH-severity findings (#171 a11y · #174/#177 usability · #179 states · #94bc049 navigation · #737cb5f error-prevention) · full-screen mis-ejercicios/mensajes (#183) · custom exercise muscle taxonomy unification con secondary muscle + equipment (#186 / `51973bc`). **Next**: smoke en device del build #3 con los 10 PRs nuevos (foco en bidirectional set-logs PT↔athlete y exercise-notes que necesitan setup de cuentas linkeadas trainer↔athlete; avatar cropper ya smoke-validated solo del editor del athlete) → iteración de bugs descubiertos en smoke → eventualmente External Testing (requiere Privacy Policy URL + Beta App Review de Apple ~24h). Después: pagos (Mercado Pago/Stripe), verificación profesional, deep links, screenshots para store, Gemini Excel (cuando GCP esté activo), group chats, video calls. **Follow-ups operativos abiertos**: trainer editor data inconsistency descubierta en smoke Vicente (`routine.name` desincronizado de `routine.days`) · CI auto-deploy de `firestore.indexes.json` para no repetir el bug de feed roto · cleanup de 2 indexes huérfanos en `posts` sin `createdAt` · restore widget-level test ajustes/CAMBIAR FOTO con cropper inyectable · home today's routine card edge case cuando el athlete usa unlock-all-days y salta días (sigue calculando `lastFinished + 1`) · limpiar dead branches `if (locked)` en routine_detail_screen que quedaron unreachable después de #205. | ✅ build #3 delivered 2026-06-29 | Proyectada ~2026-06-30 para TestFlight Internal estable — alcanzada |
