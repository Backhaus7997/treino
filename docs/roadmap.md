...
|---|---|---|---|---|---|
| 1 | **Trainer profile UI** ✅ — el PF completa su perfil público desde la app | ya en main (`ProfileEditTrainerScreen` + ruta `/profile/edit-trainer`) | Ninguno — usa `trainerPublicProfiles` ya deployado | **Hecho.** `ProfileEditTrainerScreen` (1057 líneas) edita `trainerBio`, `trainerSpecialty` (dropdown), `trainerMonthlyRate`, ubicaciones (geocode + permisos) y clases virtuales; entry point en el tab Perfil (`profile_trainer_section` → `/profile/edit-trainer`). Guardar llama `UserRepository.update` que hace el **dual-write `users/{uid}` + `trainerPublicProfiles/{uid}`** vía `WriteBatch` (SCENARIO-423/424, testeado). Discovery lee `trainerPublicProfiles`. Verificado end-to-end 2026-06-04. NOTA: el "onboarding guiado dentro de ProfileSetup" del plan NO aplica — los entrenadores se crean manualmente (Admin SDK / `promote_user_to_trainer.js`), no se auto-registran; una vez trainer, completan el perfil desde la app. Avatar/nombre se editan desde Datos personales. | B |
...
| Fase 6 (Producto-ready: trainer UI + push + agenda recurrente + telemetría + reviews + i18n) | 🔄 En curso — Etapa 6 telemetría cerrada (Crashlytics #108, Analytics #109, App Check #110); Etapa 7 reviews cerrada 2026-06-02 (`trainer-reviews`); **Etapa 4 recurring cerrada 2026-06-04 (entregada trainer-driven eager + cancel-serie, distinto al plan lazy)**; **Etapa 1 trainer profile UI verificada ya-hecha 2026-06-04** (edit screen + dual-write + discovery, roadmap estaba desactualizado); Etapa 9 self-routines + supersets (#130); resto pendiente (Etapa 2 push en curso, 3, 5, 8) | ~2026-08-21 | Proyectada ~2026-06-15 |
