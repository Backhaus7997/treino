...
## 0. Por qué existe y cómo se midió
...
1. El universo de stores se enumeró con `rg '^\s*match /' firestore.rules` →
   **34 paths** (el mismo número que reporta [security.md §1.1](./security.md)),
...
[§4, QA-CMP-011](#4-huecos-de-borrado-detectados).
...
### 1.1 Firestore — 35 stores (34 con `match` + `audit_log`)
...
### 1.3 Agregados y denormalizaciones
...
## 2. Contraste contra el cascade de borrado
...
| 15 | `chats/*/messages` | ⚪ retención deliberada | Ídem. **Pero su media sí se borra** → ver §2.3 |
...
| 33 | `reviews` | ⚪ retención deliberada | **Pero el alcance escrito no coincide con el código** → §3.2 |
...
### 2.3 Inconsistencia interna que conviene mirar
...
## 3. Contraste contra la Política de Privacidad
...
### 3.1 Lo que el código guarda y la política no declara
...
| **Gimnasio al que asiste** | `users.gymId`, `userPublicProfiles.gymId`/`gymName` | Media — ver §3.3 |
...
### 3.2 Lo que la política promete y el código no cumple
...
- **Los 6 huecos y los 2 parciales de §2.2**, que no tienen ni siquiera decisión.
...
### 3.3 Divergencia de ubicación (las dos direcciones)
...
### 3.4 Declarado pero no guardado
...
## 4. Huecos de borrado detectados
...
## 6. Cómo mantener este inventario

Mismo contrato que la matriz de [security.md §1.8](./security.md): **el PR que
cambia el dato actualiza el documento**.

1. **Campo nuevo con PII en una colección existente** → fila actualizada en §1.1
   o §1.2, y revisar si el cascade que ya cubre esa colección lo alcanza.
2. **Colección o path de Storage nuevo** → fila nueva en §1, fila nueva en la
   tabla de cobertura de §2.1 y **recalcular los conteos de §2.2**. Son
   conteos, no impresiones.
3. **Paso nuevo en el cascade** → mover el ítem a ✅ en §2.1 y, si cierra un
   `QA-CMP-xxx`, tacharlo en §4 dejando la referencia al PR.
